...
.navbar .navbar-menu-wrapper .search-field .input-group input {
  ...
  for (Contact item : conlist) { %>
    ...
    <tr>
      ...
      <td><a href="./PhoneBookController?Action=TOEDITCONTACTADMIN&conid=<%=item.getConId()%>&uidbyadmin=<%=item.getuId()%>">Edit</a>/<a onclick="return confirm('Are You Sure?')" href="./PhoneBookController?Action=DELETECONTACTADMIN&conid=<%=item.getConId()%>&uidbyadmin=<%=item.getuId()%>">Delete</a>
       <td><%
      if(item.getConIsFav()==0)
      {
    	 %>
    	 <a href="./PhoneBookController?Action=FAVOURITEADMIN&isfav=<%=item.getConIsFav()%>&conid=<%=item.getConId()%>&uidbyadmin=<%=item.getuId()%>"><img class="favimage" src="./pages/ui-features/unfav.png" alt="notfav"></a>
      ...
      {
    	  %>
    	  <a href="./PhoneBookController?Action=FAVOURITEADMIN&isfav=<%=item.getConIsFav()%>&conid=<%=item.getConId()%>&uidbyadmin=<%=item.getuId()%>"><img class="favimage" src="./pages/ui-features/fav.jpeg"  alt="not"></a>
