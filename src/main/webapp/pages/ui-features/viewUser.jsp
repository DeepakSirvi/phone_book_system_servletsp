...
.navbar .navbar-menu-wrapper .search-field .input-group input {
  ...
  for (User item : ulist) { %>
    ...
    <tr>
      <td><%=item.getuName()%></td>
      <td><%=item.getuEmail()%></td>
      <td><%=item.getuMobile()%></td>
      <td><%=item.getuAddress()%></td>
      <td><%=item.getuRole() %></td>
       <td><a href="./PhoneBookController?Action=TOADMINEDITUSER&uidbyadmin=<%=item.getuId()%>">Edit</a>/<a onclick="return confirm('Are You Sure?')" href="./PhoneBookController?Action=ADMINDELETEUSER&uidbyadmin=<%=item.getuId()%>">Delete</a>
      <td><a href="./PhoneBookController?Action=ADMINVIEWCONTACT&uidbyadmin=<%=item.getuId()%>">VIEW</a></td>
      <%-- <td><%
      if(item.getuFav()==0)
      {
    	 %>
